...
from . import css as css, runtime_data as rtd, dbcon as dbcon
...
class CurrentBeverageBtn(pyw.QPushButton):
    ...
    def __init__(
        self, __parent: pyw.QWidget, __currentBvgOnHopper: rtd.Beverage, __hopperid: int
    ):
        ...
        self.clicked.connect(
            lambda: __parent.hopperBtn_onClick(self.currentBvgOnHopper, __hopperid)
        )
        self.setStyleSheet(
            f"background-color: {css.m_sndButtonBackgroundColor}; color: {css.m_standardTextColor}; border: 1px solid {css.m_borderColor}; padding-top: 20%;padding-bottom: 20%;padding-left: 40%; padding-right: 40%;"
        )
...
class BeveragePushButton(pyw.QPushButton):
    ...
    def __init__(
        ...
        self.parentWidget = __parent
        self.setStyleSheet(
            f"background-color: {css.m_sndButtonBackgroundColor}; padding-top: 70%; padding-bottom: 70%; color: {css.m_standardTextColor}; margin: 5%;"
        )
        self.clicked.connect(
            lambda: __parent.qsDrinkButton_onClick(
                __currentBvgID, __beverageData.m_id, hopperid
            )
        )
...
class ChangeDrinkWindow(pyw.QWidget):
    ...
    def initWidgets(self):
        ...
        self.headerLabel.setStyleSheet(
            f"color: {css.m_standardTextColor}; font-size: 24pt; font-family: {css.font};"
        )
        ...
        self.informationLabel.setStyleSheet(
            f"color: {css.m_standardTextColor}; font-size: 12pt; font-family: {css.font}; margin-top: 40%;"
        )
        ...
        self.saveBtn.setStyleSheet(
            f"background-color: {css.m_buttonBackgroundColor}; color: {css.m_standardTextColor}; border: 1px solid {css.m_borderColor}; padding-top: 30%;padding-bottom: 30%;padding-left: 50%;padding-right: 50%;"
        )
        ...
        self.allDrinksFrame.setStyleSheet(
            f"border: 1px solid {css.m_borderColor}; color: {css.m_standardTextColor};"
        )
    ...
    def qsDrinkButton_onClick(self, currentID: int, toChangeID: int, hopperid: int):
        ...
        assuranceWindow.setStyleSheet(
            f"color: {css.m_standardTextColor}; background-color: {css.m_mainBackgroundColor};"
        )
            ...
            for i in range(len(self.parentWidget.rtData.m_beverageList)):
                if currentID == self.parentWidget.rtData.m_beverageList[i].m_id:
                    ...
                    temp = CurrentBeverageBtn(
                        self.parentWidget,
                        rtd.beverage(-1, hopperid, "EMPTY SLOT", 0),
                        hopperid,
                    )
                    ...
                    self.informationLabel.setText(
                        f"Removed {self.parentWidget.rtData.m_beverageList[i].m_name} from Hopper {hopperid}"
                    )
            ...
            if currentID == -1:
                ...
                for i in range(len(self.m_allBeverages)):
                    if toChangeID == self.m_allBeverages[i].m_id:  # found it
                        ...
                        self.informationLabel.setText(
                            f"Put {tempBvg.m_name} on hopper {hopperid}"
                        )
                        self.m_currentBvg = tempBvg
                        self.parentWidget.hopperList[hopperid] = CurrentBeverageBtn(
                            self.parentWidget, tempBvg, hopperid
                        )
                        break
            else:
                # find old beverage
    ...
    def updateQuickSelect(self):
        ...
        for i in range(len(self.m_allBeverages)):
            self.scrollAreadVBox.addWidget(
                BeveragePushButton(
                    self, self.m_allBeverages[i], self.m_currentBvg.m_id, self.hopperid
                )
            )
        self.scrollAreadVBox.addWidget(
            BeveragePushButton(
                self,
                rtd.beverage(-1, self.hopperid, "EMPTY SLOT", 0),
...
class EditHoppers(pyw.QWidget):
    ...
    def initWidgets(self):
        ...
        for i in range(len(self.rtData.m_beverageList)):
            temp = CurrentBeverageBtn(
                self,
                self.rtData.m_beverageList[i],
                self.rtData.m_beverageList[i].m_hopperid,
            )
