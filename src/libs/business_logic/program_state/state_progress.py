...
class StateProgress:
    ...
    def call_ui(self) -> dict:
        ...
        if self.__time_waited < self.__max_waiting_time:
            time.sleep(1)
            return self.__ui_object.select_view(self.__PROGRAM_STATE, self.__progress_percentage)
        else:
    ...
    def execute_command(self, __recipe_name: str):
        ...
        if self.__expected_weight == 0:
            self.__expected_weight = int(
                self.__hopper_object.get_expected_weight() * self.__TOLERANCE_WEIGHT_MULTIPLIER)

        __current_weight: int = self.__scale_object.get_weight()
        self.__progress_percentage: int = int((__current_weight / self.__expected_weight) * 100)
