...
class CurrentBeverageButton(PyQtWidgets.QPushButton):
    def __init__(
        ...
        __current_beverage_on_hopper: RuntimeData.Beverage,
        __hopper_id: int,
    ):
        ...
        self.parentWidget = __parent
        self.clicked.connect(
            lambda: __parent.hopper_button_on_click(
                self.current_beverage_on_hopper, __hopper_id
            )
        )
...
class ChangeDrinkWindow(PyQtWidgets.QWidget):
    def __init__(
        self,
        __parent: PyQtWidgets.QWidget,
        __runtime_data_beverage: RuntimeData.Beverage,
        __hopper_id: int,
    ):
        super().__init__()
        self.runtime_data_beverage: RuntimeData.Beverage = __runtime_data_beverage
        self.beverages_not_on_hopper: list[
            RuntimeData.Beverage
        ] = JsonData.get_all_other_beverages()
    ...
    def drink_button_on_click(self, current_id: int, to_change_id: int, hopper_id: int):
        ...
        assurance_window.setStandardButtons(
            PyQtWidgets.QMessageBox.Yes | PyQtWidgets.QMessageBox.No
        )
        ...
        if to_change_id == -1:
            ...
            for (
                beverage_on_hopper
            ) in self.parentWidget.runtime_data.beverages_on_hopper:
                if current_id == int(beverage_on_hopper.beverage_id):
                    ...
                    temp_beverage: RuntimeData.Beverage = RuntimeData.Beverage(
                        -1, hopper_id, "EMPTY SLOT", 0
                    )
                    self.runtime_data_beverage: RuntimeData.Beverage = temp_beverage
                    self.parentWidget.hopperList[hopper_id]: list[
                    ...
                    del self.parentWidget.runtime_data.beverages_on_hopper[
                        beverage_on_hopper
                    ]
        ...
        # to_change_id refers to a valid drink button
            ...
            # we are removing a non-empty drink button
                ...
                # find old Beverage
                for beverage_on_hopper in range(
                    len(self.parentWidget.runtime_data.beverages_on_hopper)
                ):
                    # found it
                    if (
                        current_id
                        ...
                        for x in range(len(self.beverages_not_on_hopper)):
                            if (
                                to_change_id
                                == self.beverages_not_on_hopper[x].beverage_id
                            ):
                                ...
                                # set new hopper_id
                                self.beverages_not_on_hopper[
                                    x
                                ].beverage_hopper_id = hopper_id
                                # override all the old data
                                self.runtime_data_beverage = (
    ...
    def update_quick_select(self):
        ...
        for i in range(len(self.beverages_not_on_hopper)):
            self.scrollAreaVBox.addWidget(
                BeveragePushButton(
                    self,
                    self.beverages_not_on_hopper[i],
...
class EditHoppers(PyQtWidgets.QWidget):
    def __init__(
        self, __parent: PyQtWidgets.QWidget, __runtime_data: RuntimeData.runtime_data
    ):
        super().__init__()
        self.m_change_drink_window: ChangeDrinkWindow
        self.parentWidget: PyQtWidgets.QWidget = __parent
        self.setWindowTitle("Edit Hopper Occupancy")
        self.runtime_data: RuntimeData.runtime_data = __runtime_data  # important
        self.resize(1200, 800)
        self.showFullScreen()
    ...
    def hopper_button_on_click(
        ...
        self.m_change_drink_window = ChangeDrinkWindow(
            self, __current_beverage_on_hopper, __hopper_id
        )
        self.hide()
