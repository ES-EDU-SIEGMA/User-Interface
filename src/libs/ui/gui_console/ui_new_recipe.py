class NewRecipe:
    ...
    def activate(self, __ingredient_names: list[str], __recipe_names: list[str]) -> list[str]:
    ...
    def __try_split_string_command(self, __input: str):
        ...
        # 2.command is [<ingredient-name>;<ml-amount>]
    ...
    def __try_add_ingredient(self, __split_input: list[str]):
        if __split_input[0] in self.__ingredient_names and __split_input[1].isdigit():
            ...
            if (self.__sum_fill_amounts + __fill_amount) < self.MAX_FILL_AMOUNT:
                self.__new_recipe.append(__ingredient_name)  # appending ingredient name
                self.__new_recipe.append(str(__fill_amount))  # appending fill amount ml
