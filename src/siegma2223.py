...
class beveragePushButton(pyw.QPushButton):
    ...
    def __init__(self, __parent: pyw.QWidget, __beverageData: rtd.beverage):
        ...
        self.setStyleSheet(
            f"background-color: {css.m_sndButtonBackgroundColor}; padding-top: 70%; padding-bottom: 70%; color: {css.m_standardTextColor}; margin: 5%; font-size: 11pt;"
        )
        self.clicked.connect(
            lambda: self.parentWidget.qsDrinkButton_onClick(self.m_index, False)
        )
...
class mixedDrinkPushButton(pyw.QPushButton):
    ...
    def __init__(self, __parent: pyw.QWidget, __beverageData: rtd.beverage):
        ...
        self.clicked.connect(
            lambda: self.parentWidget.qsDrinkButton_onClick(self.m_index, True)
        )
        self.information = __beverageData
        self.setText(self.information.m_name)
        self.m_index = self.information.m_id
...
class welcomeWindow(pyw.QWidget):
    ...
    def initWidgets(self):
        ...
        self.headerLabel.setStyleSheet(
            f"color: {css.m_standardTextColor}; font-size: 30pt; font-family: {css.font}; margin-top: 25%;"
        )
        ...
        self.subheaderLabel.setStyleSheet(
            f"color: {css.m_buttonBackgroundColor}; font-size: 15pt; font-family: {css.font};"
        )
        self.descriptionLabel = pyw.QLabel("Quick select for drinks:", self)
        self.descriptionLabel.setStyleSheet(
            f"color: {css.m_standardTextColor}; font-size: 12pt; font-family: {css.font};"
        )
        ...
        self.exitBtn.setStyleSheet(
            f"background-color: {css.m_buttonBackgroundColor}; color: {css.m_standardTextColor}; padding: 60%; font-size: 11pt; border: 2px solid {css.m_borderColor}; border-radius: {css.borderRadius}px; margin-left: 20%;"
        )
        ...
        self.editHopperOccupancyBtn.clicked.connect(
            lambda: self.editHopperOccupancyBtn_onClick()
        )
        ...
        self.allDrinksFrame.setStyleSheet(
            f"border: 0px solid {css.m_borderColor}; color: {css.m_standardTextColor};"
        )
    ...
    def updateQuickSelect(self):
        ...
        for i in range(len(self.m_runtimeData.m_mixeddrinkList)):
            self.scrollAreadVBox.addWidget(
                mixedDrinkPushButton(self, allMixedDrinks[i])
            )
    ...
    def qsDrinkButton_onClick(self, index: int, mixeddrink: bool):
        ...
        if mixeddrink:
            self.progressWindow = spw.mixingProgressWindow(
                self, None, self.m_runtimeData.getMixedDrinkToId(index)
            )
        else:
            self.progressWindow = spw.mixingProgressWindow(
                self, self.m_runtimeData.getBeverageToId(index), None
            )
    ...
    def integrationTest(self):
        ...
        for i in range(len(self.m_runtimeData.m_beverageList)):
            ...
            hoppers[currentHopper % 4] = int(
                go.standardActivationTime
                * self.m_runtimeData.m_beverageList[i].m_flowspeed
                * 1000
            )
            picoid = 0
            if currentHopper < 5:
                ...
                picoid = 2
            sc.send_msg(
                picoid, f"{hoppers[0]};{hoppers[1]};{hoppers[2]};{hoppers[3]};\n"
            )
